...
def load_data(nrows):  
    ...
    df_desc = pd.read_csv("PAD_04_dash_desc.zip", compression='zip', nrows=nrows)
    df_pred = pd.read_csv("PAD_04_dash_pred.zip", compression='zip', nrows=nrows)
    df_imp = pd.read_csv("PAD_04_feature.csv")
    df_cst = pd.read_csv("PAD_04_cost.csv")
...
def lire_ligne():
    file=open("PAD_05_ligne.txt", "r")
    line=file.read()
    file.close()
    return line

def ecrire_ligne(line):
    file=open("PAD_05_ligne.txt", "w+")
    file.write(str(line))
    file.close()
